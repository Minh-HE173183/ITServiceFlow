 ...
 */
@WebServlet(name = "WorkflowController", urlPatterns = { "/workflows" })
public class WorkflowController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (!AuthUtils.isLoggedIn(req, resp))
            return;

        // Check authorization (e.g., Admin, IT Director, System Engineer can manage
        // workflows)
        if (!AuthUtils.hasRole(req, resp, AuthUtils.ROLE_ADMIN, AuthUtils.ROLE_IT_DIRECTOR,
                AuthUtils.ROLE_SYSTEM_ENGINEER))
            return;
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        // Check authentication & authorization for POST actions
        if (!AuthUtils.isLoggedIn(req, resp))
            return;
        if (!AuthUtils.hasRole(req, resp, AuthUtils.ROLE_ADMIN, AuthUtils.ROLE_IT_DIRECTOR,
                AuthUtils.ROLE_SYSTEM_ENGINEER))
