    ...
    <body>
        <div class="container">
            ...
            <form
                ...
                method="post">
                <c:if test="${not empty incident}"><input type="hidden" name="id" value="${incident.ticketId}">
                </c:if>
                ...
                <c:if test="${not empty incident}">
                    <div class="form-group">
                        ...
                        <select id="status" name="status">
                            ...
                            <option value="IN_PROGRESS" ${incident.status=='IN_PROGRESS' ? 'selected' : '' }>In
                                Progress</option>
                            <option value="RESOLVED" ${incident.status=='RESOLVED' ? 'selected' : '' }>Resolved
                            </option>
                            <option value="CANCELLED" ${incident.status=='CANCELLED' ? 'selected' : '' }>Cancelled
                            </option>
                        </select>
                    </div>
                </c:if>
                ...
                <c:if test="${empty incident}">
                    <div class="form-group">
                        ...
                        <input type="text" id="relatedIds" name="relatedIds" placeholder="101,102,103">
                        <div class="help-text">Optional – associate this ticket with existing incidents.</div>
                    </div>
                </c:if>

                <button type="submit" class="btn">${not empty incident ? 'Save Changes' : 'Submit
                                                    Incident'}</button>
